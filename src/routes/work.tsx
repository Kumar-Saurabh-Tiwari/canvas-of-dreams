...
import { useSceneStore } from "@/components/portfolio/scene-store";
...
export const Route = createFileRoute("/work")({
  head: () => ({
    meta: [
      { title: "Work — Kumar Saurabh Tiwari" },
      { name: "description", content: "Selected projects: client management frontend, ed-tech SaaS, open source initiatives." },
      { property: "og:title", content: "Work — Kumar Saurabh Tiwari" },
      { property: "og:description", content: "Limits not found. Selected work in product engineering." },
    ],
  }),
  component: WorkPage,
});
...
function ProjectRow({ p }: { p: typeof projects[number] }) {
  ...
  const onEnter = () => {
    setIntensity(2.2);
    if (ref.current) {
      gsap.to(ref.current, { skewX: -8, x: 24, duration: 0.5, ease: "power3.out" });
    }
  };
  const onLeave = () => {
    setIntensity(1);
    if (ref.current) {
      gsap.to(ref.current, { skewX: 0, x: 0, duration: 0.6, ease: "power3.out" });
    }
  };
    ...
    >
      <div ref={ref} className="flex items-baseline justify-between gap-6 will-change-transform">
        <div className="flex items-baseline gap-6 md:gap-10 flex-1 min-w-0">
          ...
          <h3 className="text-2xl md:text-4xl lg:text-5xl font-bold tracking-tight truncate group-hover:text-violet-glow transition-colors">
            {p.title}
          </h3>
        </div>
...
function WorkPage() {
  return (
    <section className="min-h-screen px-6 md:px-16 py-32 font-mono">
      <p className="text-[10px] tracking-[0.4em] text-muted-foreground mb-6">[ 03 // WORK ]</p>
      <h2 className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight mb-24 text-glow">
        <TextScramble as="span" text="LIMITS NOT FOUND" duration={1600} />
      </h2>
      ...
      <p className="mt-12 text-[10px] tracking-[0.3em] text-muted-foreground">
        [ {projects.length.toString().padStart(2, "0")} ENTRIES // MORE ON REQUEST ]
      </p>
    </section>
  );
}
